...
Vue blog — 통합 멀티플랫폼 블로그 에이전트 지침서 (PART 0~O 전문)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
[이곳에 대표님이 주신 7000자 지침 전문이 무삭제로 삽입되어 AI에게 전달됩니다.]
핵심: 인라인 스타일만 사용, 금지 표현 절대 불가, 4,000자 이상, 4개의 [이미지 삽입] 위치 전략적 배치!
코드블록 밖에 이미지 프롬프트 ①~④번 번호와 함께 출력 필수.
`;

async function callAI(model, prompt, retry = 0) {
    try {
        const r = await model.generateContent('[SYSTEM: ACT AS AN ELITE CONTENT STRATEGIST. NO CHAT. FOLLOW ALL RULES EXACTLY.]\n\n' + prompt);
...
async function writeAndPost(model, target, blogger, bId, pTime, idx) {
    console.log(`\n==================================================`);
    console.log(`💎 [VUE v4.2] ${idx}/5 집필 프로세스 가동: ${target}`);
    console.log(`==================================================`);
    ...
    for(let i=0; i<phMatches.length; i++) {
        ...
        const imgHtml = `<div style="text-align:center; margin:45px 0;">
            <img src="${url}" alt="${at}" title="${tt}" style="max-width:100%; border-radius:15px; box-shadow:0 12px 35px rgba(0,0,0,0.1);">
            <p style="font-size:14px; color:#888; margin-top:12px;">${tt}</p>
        </div>`;
    ...
    } catch(e) { console.error(`🚨 발행 실패: ${e.message}`); throw e; }
}
async function run() {
    try {
    ...
    } catch(e) { console.error('\n🚨 가동 실패:', e.message); process.exit(1); }
}
run();