...
function clean(raw, defType = 'obj') {
    ...
    if (defType === 'text') {
        t = t.replace(/<!DOCTYPE.*?>|<html.*?>|<\\/html>|<head.*?>[\\s\\S]*?<\\/head>|<body.*?>|<\\/body>|<title.*?>[\\s\\S]*?<\\/title>/gi, '');
        return t.trim();
    }
    try { const s = t.indexOf('{'); const e = t.lastIndexOf('}'); if(s!==-1 && e!==-1) return t.substring(s, e+1); } catch(e){}
    return '{}';
}
async function callAI(model, prompt) { try { const r = await model.generateContent(prompt); return r.response.text().trim(); } catch(e){ if(e.message.indexOf('429')!==-1) { await new Promise(r=>setTimeout(r,30000)); return callAI(model, prompt); } throw e; } }
...
async function writeAndPost(model, target, blogger, bId, pTime) {
    ...
    const bpRes = await callAI(model, `Return ONLY JSON for topic "${target}": {"title":"...","chapters":["ch1","ch2","ch3","ch4","ch5","ch6","ch7"]}`);
    ...
    for(let i=0; i<chapters.length; i++) {
        const sect = clean(await callAI(model, `Write SEO body for: ${chapters[i]} (Topic: ${target}). NO HTML HEADERS. HTML tags like p, ul, table ONLY.`), 'text');
        body += `<div class="h2-premium"><h2>${chapters[i]}</h2></div>` + sect;
    }
    body += '</div>';
...
async function run() {
    ...
    const pool = config.clusters || []; if(!pool.length) return console.log('Empty');
    const mainSeed = pool.splice(Math.floor(Math.random()*pool.length), 1)[0];
    await writeAndPost(model, mainSeed, blogger, config.blog_id, new Date());
