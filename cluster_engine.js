...
async function genImg(desc, model, sectionIdx) {
    ...
    try {
        const cr = await axios.post('https://api.kie.ai/api/v1/jobs/createTask', { model: 'z-image', input: { prompt: desc + ', realism, 8k', aspect_ratio: '16:9' } }, { headers: { Authorization: 'Bearer ' + process.env.KIE_API_KEY } });
        ...
        for(let i=0; i<15; i++) { 
            ...
            const data = pr.data.data || pr.data; if(data.state === 'success') { 
...
async function genHyperRealThumbnail(keyword, model) {
    ...
    const l2 = String(d.line2 || 'Expert Guide').substring(0, 20);
    ...
    return await uploadToCloudinary(Buffer.from(svg), "THUMB_STABLE");
}

async function writeAndPost(model, target, blogger, bId) {
    console.log('\n🛡️ [VUE Master] Stabilized Core 가동...');
    const mktPrompt = `억대 연봉 마케터입니다. 키워드 "${target}"를 위한 제목과 7개 섹션 목차를 짜세요. 목차(chapters)는 반드시 순수 텍스트 배열이어야 합니다. JSON: { "long_tail_keyword":"", "title":"", "chapters":["섹션1", "섹션2", "섹션3", "섹션4", "섹션5", "섹션6", "FAQ"] }`;
    ...
    // 🛡️ [Data Normalizer] 목차 무결성 강제 정규화
    const rawChapters = bp.chapters || [];
    const chapters = rawChapters.map(c => typeof c === 'object' ? (c.title || c.chapter || c.name || String(c)) : String(c));
    ...
    for(let i=0; i<chapters.length; i++) {
        ...
        console.log(`      ㄴ [${isFAQ ? 'FAQ' : '집필'}] ${i+1}/7: ${chapters[i]}`);
        let sectPrompt = isFAQ ? `마케팅/CS 전문가로서 [${chapters[i]}]를 5가지 핵심 Q&A(h3, p)로 구성하세요.` : `억대 연봉 마케터로서 [현재 섹션: ${chapters[i]}] 내용을 HTML 태그로 2,500자 이상 작성하세요. 제목: ${title}. 이전 중복 금지: ${cumulativeCtx}. [IMAGE_PROMPT: 영어] 포함. 표(table) 포함.`;
        ...
        body += `<div class="h2-premium" id="s${i+1}"><span class="premium-chip">${isFAQ ? 'FAQ SPECIAL' : 'INSIGHT 0' + (i+1)}</span><h2>${chapters[i]}</h2></div>` + htmlSect;
    }
...
async function run() {
    try {
        ...
        await axios.put(`https://api.github.com/repos/${process.env.GITHUB_REPOSITORY}/contents/cluster_config.json`, { message: 'Stable Sync', content: Buffer.from(JSON.stringify({...config, clusters: pool}, null, 2)).toString('base64'), sha: g.data.sha }, { headers: { Authorization: 'token '+process.env.GITHUB_TOKEN } });
