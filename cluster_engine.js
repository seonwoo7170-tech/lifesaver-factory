...
`;
  ...
  content = content.replace(/[[IMG_(MID)?1]]/g, wrapImg(imgTop)).replace(/[[IMG_(MID)?2]]/g, wrapImg(imgMid1)).replace(/[[IMG_(MID)?3]]/g, wrapImg(imgMid2)).replace(/[[IMG_(BTM)?4]]/g, wrapImg(imgBtm));
  ...
  await blogger.posts.insert({blogId: bId, requestBody: {title: data.title || target, labels: labels, content: fullHtml, customMetaData: data.description || '', published: pTime.toISOString() } });
  console.log("🎉 [STEP 5] 블로그 자동 포스팅 최종 발행 성공!!!");
  console.log("=========================================");
}
