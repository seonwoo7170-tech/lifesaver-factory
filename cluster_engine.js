...
async function writeAndPost(model, target, lang, blogger, bId, pTime, extraLinks = [], idx, total) {
    ...
    if(m0) finalHtml = finalHtml.replace(m0[0], '').trim();
    ...
    for(let i=1; i<=4; i++) {
        let imgPrompt = target + ' professional photo ' + i;
        let imgAlt = 'Image ' + i, imgTitle = '';
        const imgRegex = new RegExp('IMG_' + i + ':[^{]*\\{([^}]*)\\}', 'i');
        const m = fullRaw.match(imgRegex);
        if(m) {
            finalHtml = finalHtml.replace(imgRegex, '').trim();
            const pMatch = m[0].match(/prompt:\s*["\'](.*?)["\']/i);
            const aMatch = m[0].match(/alt:\s*["\'](.*?)["\']/i);
            const tMatch = m[0].match(/title:\s*["\'](.*?)["\']/i);
            if(pMatch) imgPrompt = pMatch[1];
            if(aMatch) imgAlt = aMatch[1];
            if(tMatch) imgTitle = tMatch[1];
        }
        if(finalHtml.includes('[[IMG_'+i+']]')) {
            const url = await genImg(imgPrompt, model, i);
            finalHtml = finalHtml.split('[[IMG_'+i+']]').join(`<img src='${url}' alt='${imgAlt}' title='${imgTitle}' style='width:100%; border-radius:12px; margin:30px 0;'>`);
        }
    }
