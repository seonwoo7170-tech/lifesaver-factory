...
async function writeAndPost(model, target, lang, blogger, bId, pTime, extraLinks = [], idx, total) {
    ...
    let mission1 = "TRINITY MISSION 1 (Part 1/3): H1 Title + TOC + Intro + S1-3. NARRATIVE: \\\"" + NARRATIVE_HINTS + "\\\". S1: \\\"" + chapters[0] + "\\\" (" + vLogicRes[0] + "), S2: \\\"" + chapters[1] + "\\\" (" + vLogicRes[1] + "), S3: \\\"" + chapters[2] + "\\\" (" + vLogicRes[2] + "). RULES: Each section 1,200~1,500+ chars (Part 1 total 4,000+), H2 IDs s1-s3. START WITH H1 TITLE.";
    let part1 = await callAI(model, "STRICT: " + MASTER_GUIDELINE + "\\n\\n" + mission1 + "\\n\\nSearch: " + searchData);

    let mission2 = "TRINITY MISSION 2 (Part 2/3): S4-7. S4: \\\"" + chapters[3] + "\\\" (" + vLogicRes[3] + "), S5: \\\"" + chapters[4] + "\\\" (" + vLogicRes[4] + "), S6: \\\"" + chapters[5] + "\\\" (" + vLogicRes[5] + "), S7: \\\"" + chapters[6] + "\\\" (" + vLogicRes[6] + "). RULES: Each section 1,200~1,500+ chars (Part 2 total 4,000+), H2 IDs s4-s7. DO NOT REPEAT ANY INFORMATION OR PHRASES FROM PREVIOUS PART. CONTINUE THE NARRATIVE FLOW SEAMLESSLY.";
    let part2 = await callAI(model, "STRICT: " + MASTER_GUIDELINE + "\\n\\n[ALREADY WRITTEN CONTENT]:\\n" + part1.substring(Math.max(0, part1.length - 8000)) + "\\n\\n[MISSION]:\\n" + mission2);

    let mission3 = "TRINITY MISSION 3 (Part 3/3): FAQ (10 items) + Disclaimer + Closing + Tags + Schema + PART O METADATA. RULES: 10,000+ total target chars for the whole blog. PROVIDE THE FINAL CONCLUSION BASED ON THE PREVIOUS NARRATIVE. NEVER REPEAT ALREADY WRITTEN PARTS.";
    ...
    const searchDesc = getMeta(fullContent, '📝 검색 설명');

    const imgPrompts = {};
    const imgRowRegex = /IMG_(\d+):\s*\{?\s*prompt:\s*[\"\'](.*?)[\"\'],\s*alt:\s*[\"\'](.*?)[\"\'],\s*title:\s*[\"\'](.*?)[\"\']\s*\}?/gi;
    let im;
    while ((im = imgRowRegex.exec(fullContent)) !== null) {
    ...
    for (let i = 1; i <= 4; i++) {
        ...
        if (finalHtml.includes(placeholder)) {
            const p = imgPrompts[i] || { prompt: target, alt: target, title: target };
  ...
  async function run() {
    ...
    console.log('\n🚀 연재 시도:', mainSeed);
    let subRes = await callAI(model, 'Generate exactly 7 short Korean sub-topic titles about: ' + mainSeed + '. Return ONLY a valid JSON array. Example: [\\\"a\\\",\\\"b\\\"]. No markdown.');
    let subTopics = [];
    try { subTopics = JSON.parse(clean(subRes, 'arr') || '[]'); } catch(e) {
        console.log('JSON fallback');
        subTopics = subRes.split(/\\n+/).map(s=>s.replace(/^[\\d\\.\\-\\*\\\\\\[\\\\\\]\\\\\" ]+/g,'').trim()).filter(s=>s.length>2 && s.length<80).slice(0,7);
    }
    if(!subTopics.length) subTopics=[mainSeed+' 완벽 입문 가이드', mainSeed+' 성능 최적화 세팅', mainSeed+' 주의사항'];
    console.log('📬 대상 키워드:', subTopics.length + 1, '개');
    let cTime = new Date();
    for(let i=0; i < subTopics.length; i++) {
        cTime.setMinutes(cTime.getMinutes()+180);
        console.log('📝 [' + (i+1) + '/' + (subTopics.length+1) + '] ' + subTopics[i]);
        await writeAndPost(model, subTopics[i], config.blog_lang, blogger, config.blog_id, new Date(cTime), [], i+1, subTopics.length+1);
    }
    cTime.setMinutes(cTime.getMinutes()+180);
    console.log('💎 메인 키워드 연재:', mainSeed);
