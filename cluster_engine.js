...
async function writeAndPost(model, target, blogger, bId, pTime, lang, extraPrompt = '') {
  ...
  const wrapImg = (i) => `
<div style="text-align:center; margin:35px 0;"><img src="${i.url}" alt="${i.alt}" title="${i.title}" style="width:100%; border-radius:15px;"><p style="font-size:12px; color:#888; margin-top:8px;">${i.alt}</p></div>
`;
  ...
  console.log("=========================================");
  return { title: data.title || target, url: res.data.url };
}
...
async function run() {
  ...
  if (config.post_mode === 'cluster') {
    console.log("====== 풀 클러스터 모드 가동 시작 (4 Sub + 1 Main) ======");
    const subPosts = [];
    const maxSubs = Math.min(4, config.clusters.length);
  ...
  const extraPrompt = "\n\n[CLUSTER_MAIN_PILLAR_DIRECTIVE]: You are writing the MAIN PILLAR post that connects " + subPosts.length + " sub-posts.\n" +
    ...
    "**LENGTH RULE**: The MAIN PILLAR post must cover the sub-posts deeply, but you MUST stay within the model's maximum output limit (approx. 5,500 ~ 6,500 Korean characters). Do NOT be so verbose that you truncate the JSON response mid-way. Provide maximum possible detail while ensuring the JSON structure completes perfectly without syntax errors.\n" +
